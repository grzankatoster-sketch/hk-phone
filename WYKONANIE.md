...
### 4.26 Kokpit operatora SaaS „Hotele" — lista, opłacone-do, kill-switch, numer WhatsApp per hotel
**Jak ma działać:** nowa rola `superadmin` w `app_accounts` (analogicznie do `owner` z 2.13) — konto operatora SaaS (usera, nie hotelu), logowanie tym samym mechanizmem co reszta panelu (Supabase Auth + `app_accounts`). **Osobna strona `public/hk-phone/superadmin.html`, NIE zakładka w panel.html** (świadoma decyzja usera: „to miało być tylko dla mnie... musi być osobne okno" — panel.html to produkt sprzedawany hotelom, kokpit operatora musi być poza nim). Strona ma własne logowanie (imię+hasło, wspiera pierwsze logowanie przez `signUp`+`claim_account`) i po zalogowaniu twardo weryfikuje `current_app_role()==='superadmin'` — jeśli nie, wylogowuje i pokazuje odmowę dostępu. Widok: lista `tenants` (nazwa, slug, status, plan, `trial_ends_at`, `whatsapp_number`) przez RPC `superadmin_list_tenants`; akcje: kill-switch (`superadmin_set_tenant_status` → `trial`/`active`/`suspended`), zmiana daty opłacone-do (`superadmin_set_tenant_trial_ends`), numer WhatsApp per hotel (`superadmin_set_tenant_whatsapp` — decyzja usera: każdy hotel ma WŁASNY dedykowany numer/telefon, nie jeden wspólny bot), dodanie nowego hotelu (`superadmin_create_tenant`, 7 dni próby domyślnie). Write-policy na `tenants` (0049) zawężona z „każda zalogowana rola" do wyłącznie `superadmin` (0056) — to była realna luka, zanim kill-switch miał zęby. Bootstrap pierwszego konta `superadmin` WYŁĄCZNIE przez ręczny SQL insert (nie ma UI do samo-nadania tej roli — `admin_add_account`/`admin_set_role` celowo NIE mają `superadmin` na białej liście dozwolonych ról, więc hotelowy `admin` nie może sam sobie/nikomu jej nadać nawet próbując bezpośrednio przez RPC).
**Kryterium gotowości:** `superadmin.html` (osobny URL, bez linku z panel.html) pokazuje pełną listę hoteli po zalogowaniu kontem z rolą `superadmin`; próba zalogowania tym kontem gdziekolwiek indziej (panel.html) nie daje dostępu do niczego ponad standardowe zakładki (rola nieużywana w `tabsFor`); kliknięcie „Wyłącz teraz" ustawia `status='suspended'`; próba zapisu do `tenants` kontem innej roli odrzucona przez RLS; próba nadania roli `superadmin` przez `admin_add_account`/`admin_set_role` kończy się błędem „Zła rola".
**Dział mózgu:** DEV (mozg-dev) implementuje migrację + osobna strona. Zależność: wymaga 2.1 (tenants, już wdrożone 0049); komplementarne do 5.2 (kill-switch mechanizm + płatności) — to jest brakująca UI-połowa tamtej pozycji, nie duplikat. 4.24 (bot WhatsApp) przechodzi z architektury „jeden wspólny numer" na „jedna sesja Baileys per tenant" — więcej fizycznych telefonów/SIM-ów do utrzymania, ale zgodne z decyzją usera.
